...
4. **맞추기** — 검지 끝을 마커 중앙에 댄 채 **Snap hand to marker**를 누른다. 오차가 그대로 보정값(`Hand offset`
   3칸)이 되어 `H` 스켈레톤과 **플러그 끝 추정**(§12-4)이 같이 옮겨지고 `Tuning.json`에 저장된다. 다시 1을 해서 1cm 안쪽인지 본다
...
## 12. 실제 손으로 USB 꽂기 (2026-09-13 추가, 09-21 손 모양 구멍 제거)

**왜 하는가**: MR에서는 가상이 **깊이와 상관없이** 카메라 영상 위에 그려진다. 그래서 가상 콘솔을 쓰면
**실제 손이 가상 콘솔에 가려진다.** 이 프로젝트는 가상 손을 그리지 않는다.

**2026-09-21부터 손을 되살리는 방법은 깊이 추정(12-3) 하나다.** 손 모양 구멍은 제거했다(12-2).
**가상 콘솔 위에서는 손도 플러그도 안 보이는 것이 정상**이고, 꽂혔는지는 **포트 표식(12-4)이 알려준다.**
...
끊기는 자세를 적어 둔다. **12-4 포트 판정은 이 추적 품질을 넘을 수 없다** — 손이 끊기면 플러그 끝도 없다.

### 12-2. 손 모양 구멍 — **2026-09-21 제거됨**

추적된 손 모양대로 가상 화면에 구멍을 뚫어 실제 손을 보여주던 기능이다. **뺐다.**

**왜**: 구멍을 추적된 관절로 만드니 **손을 움직이면 구멍이 추적 지연만큼 늦게 따라왔다.** 실제 손과 어긋난 구멍은
구멍이 아예 없는 것보다 보기 나쁘다. 플러그 모양 구멍을 먼저 뺀 것(09-16)과 같은 이유다.

**그래서 지금은**: 가상 콘솔 위에서 **실제 손도 플러그도 가려진다.** 손을 보고 싶으면 깊이 추정(12-3)뿐이고,
**꽂혔는지는 포트 표식(12-4)이 알려준다** — 이게 이 시나리오의 설계된 답이다.

**되살리려면**: 손 모양을 다시 만들지 말고 **Varjo Base의 하드웨어 occlusion**(Settings > Occlusion > Hands,
**Pro 라이선스 + Base 4.16 이상**)을 먼저 시험한다. 앱이 아니라 컴포지터가 처리하므로 지연이 다르다.
지웠던 구멍 코드는 git 히스토리에 있다(`CXMRVirtualHandComponent`, 2026-09-21 이전).

- 남은 것은 **플러그 끝 추정**뿐이다(`CXMR Plug Tip` 컴포넌트, `BP_CXMRPawn → PlugTip`). 아무것도 그리지 않고,
  포트 판정에 쓸 점 하나만 계산한다 → 12-4
- 사라진 것: `CXMR.HandCutOut`, `CXMR.HandCutOut.Preview`, 튜닝 창 `Hand cut-out` 분류,
  `Mask Padding` · `Radius Scale` · `Palm Thickness` 값. `CXMR.HandCutOut.Preview`는 **`CXMR.PlugTip.Preview`**가 대신한다
  (헤드셋 없이 PIE에서 플러그 끝을 맞출 때 쓰는 가짜 손 포즈)

### 12-3. 깊이 추정 — `T` → `U`

**실제 손을 가상 표면 앞에 보이게 하는 유일한 수단이다**(12-2를 뺀 뒤).
...
- **손이 보이든 안 보이든 동작한다.** 손 추적만 되면 판정한다 — 표식이 초록으로 뜨는 것이 손 대신 알려 주는 신호다
...
- **플러그 끝 맞추기 — 튜닝 창 `Plug tip` → `Show the plug tip line`**(콘솔 `CXMR.PlugTipDebug 1`) ★ 먼저 한다:
  판정에 쓰는 플러그 끝에서 분홍 선이 플러그 방향으로 뻗는다.
  길이는 `Plug tip ahead of the pinch`(엄지·검지 사이에서 플러그 끝까지, 기본 1.75cm), 방향·미세 위치는 `Plug Offset`이다.
  선이 **실제 플러그 끝에서 시작해** 플러그가 향한 쪽으로 뻗도록 `BP_CXMRPawn → PlugTip → Plug Offset`을 맞춘다
  (기준: 엄지 끝과 검지 끝의 가운데, X = 검지 방향, Z = 검지에서 엄지 쪽). 플러그를 쥐는 방식이 사람마다 다르다
  - `Plug hand` 줄이 지금 손이 잡히는지 알려 준다. 안 잡히면 값을 아무리 만져도 소용없다 — §8 손 추적이 먼저다
  - **헤드셋 없이 PIE에서** 맞춰 볼 때는 `Use a canned hand (no headset)`(콘솔 `CXMR.PlugTip.Preview 1`)
