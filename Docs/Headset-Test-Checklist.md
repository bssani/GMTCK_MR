...
## 12. 가상 손 vs 깊이 비교 (2026-09-13 추가)
...
끊기는 자세를 적어 둔다. 12-2 가상 손의 품질은 이 추적 품질을 넘을 수 없다.

### 12-2. 가상 손 — `CXMR.VirtualHands 1`
...
### 12-3. 실제 손 — 깊이 비교 / 손 모양 구멍

**A. 깊이 비교**: `CXMR.VirtualHands 0` → `T` → `U`
...
**B. 손 모양 구멍**(2026-09-15): `M`(MR) → `CXMR.VirtualHands 2`. 처음엔 `T`/`U`를 끈 채로 본다

추적된 손 모양대로 가상 화면에 구멍을 뚫어 **카메라에 찍힌 실제 손과 USB 케이블**을 보여준다. 깊이 추정을 쓰지 않아
A처럼 가장자리가 찢기지 않는다.
- ✅ 기대: 가상 큐브 앞에서 실제 손이 매끈한 테두리로 보인다. 손을 큐브 **안으로** 넣으면 그 부분은 가려진다
- 마스킹이 꺼져 있었으면 자동으로 켜지고, 모드를 바꾸면(`CXMR.VirtualHands 0`/`1`) 다시 꺼진다.
  로그 `Hand cut-out: masking switched on`
- 손가락 둘레로 실제 배경이 조금 같이 보이는 게 정상이다(추적 오차 여유). 폭은 `BP_CXMRPawn → VirtualHands → Mask Padding`(기본 0.5cm)
  - 손끝이 잘려 보이면 키우고, 둘레가 너무 넓으면 줄인다. 한쪽으로만 어긋나면 §8 손 보정이 먼저다
...
- 들고 있는 플러그와 케이블 앞 6cm도 구멍에 들어간다. 12-2에서 `Plug Offset`을 맞춰 두면 그대로 쓰인다
- 헤드셋 없이 모양만 볼 때: `CXMR.VirtualHands.Preview 1` + `CXMR.VirtualHands 2` → 가상 물체 위에 손 모양이 검게 뚫린다
...
- **가상 손을 꺼도 동작한다**(12-3 깊이 비교 중에도). 손 추적만 되면 판정한다
- 로그: `LogCXMRPort: Port '...' ALIGNED (plug lined up)`
- 테두리 크기 `Frame Size`(기본 1.4 × 0.9cm) / 평소엔 숨기고 맞췄을 때만 보이려면 `Show When Idle` 끄기
- **내 USB 메쉬를 표식으로 쓰기**(2026-09-15): `Custom Mesh → Indicator Mesh`에 메쉬를 넣으면 막대 4개 대신 그 메쉬 하나가 보인다.
  `Indicator Offset`으로 구멍 위에 맞춘다(표식 기준, X = 구멍 바깥). 평소엔 메쉬 자체 머티리얼, 맞추면 메쉬 전체가 `Aligned Color`(초록).
...
- 12-2에서 맞춘 `Plug Offset`이 판정에도 그대로 쓰인다. 가상 플러그가 실제 플러그와 겹치게 먼저 맞춘다

### 12-5. 결과와 함께 적을 것

- 헤드셋 PC의 **Varjo Base 버전과 라이선스(Pro 여부)**. Pro라면 Base 설정의 Occlusion > Hands도 시험한다
- 모니터의 미러 화면에 **패스스루가 같이 보이는가**. 12-3 방식은 이게 돼야 주변 사람들에게 보여줄 수 있다.
  12-2는 UE 화면만으로 보여줄 수 있다
