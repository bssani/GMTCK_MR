...
**판별 실험 (30초, 코드 수정 없음)**:
1. `L_Main`의 `PostProcessVolume_0` → `Rendering Features > Post Process Materials`에서
   **`PP_MR`을 일시 제거**
2. Play → `M` → `B`

| 결과 | 결론 |
|---|---|
| 패스스루가 뜬다 | `PP_MR`이 원인 확정 → 마스크 바깥에서 알파를 건드리지 않도록 머티리얼 수정 |
| 여전히 검정 | `PP_MR`이 아니다 → 알파 파이프라인 조사 (TSR 알파, `r.SceneColorFormat`, 백버퍼 포맷) |

⚠️ `PP_MR`은 `/CXMR/` 공용 콘텐츠다 — 고치면 **모든 프로그램에 영향**이 간다.
수정 후 `N`(마스킹) 회귀를 반드시 함께 확인한다.
