...
- 들고 있는 플러그와 케이블 앞 6cm도 구멍에 들어간다. 위치는 12-4 끝의 `Plug Offset`을 따른다
- 조정은 `BP_CXMRPawn → VirtualHands`에서 한다. 리빌드는 필요 없다(플레이를 다시 시작하면 적용)
...
- **플러그 끝 맞추기 — `CXMR.HandCutOut 0` + `CXMR.PlugTipDebug 1`** ★ 먼저 한다: 판정에 쓰는 플러그 끝에서 분홍 선이 플러그 방향으로 뻗는다.
  구멍을 켜 두면 손가락 뒤로 지나는 부분이 가려지니 맞추는 동안은 끈다.
