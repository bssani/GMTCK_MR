...
## ✅ 원인 확정됨 (2026-07-27) — 아래 판별 절차는 이제 참고용이다

**원인은 2-A, `PP_MR`이다.** 필립님이 실기에서 **PostProcessVolume의 PP_MR 머티리얼 값을 0으로
내리자 하늘이 검게 변하고 MR이 동작**했다. 이것이 2-A 절차가 시키는 A/B 테스트 그 자체이고,
결과는 명확하다 — **PP_MR을 빼면 패스스루가 나온다.**

세 갈래 증거가 같은 곳을 가리킨다:
1. 실기 A/B — 머티리얼을 끄면 동작
2. 머티리얼 레시피(§19/§4-1) — *"`CustomDepth < SceneDepth` → Opacity 0, **아니면 1.0**"*
   → 마스크 구멍 바깥 **전 영역**의 알파를 1로 덮어쓴다
3. 애셋 확인 — `PP_MR`은 `Opacity`를 출력하고 `BlendableOutputAlpha`가 켜져 있다

**왜 이런 머티리얼이었나**: Varjo 예제는 **불투명한 가상 방**을 전제로 거기에 구멍을 뚫는다.
그 전제에서는 바깥 알파를 1로 두는 게 옳다. CXMR은 반대로 **배경 자체가 패스스루**여야 하므로
같은 레시피가 정면으로 충돌한다. 예제에서 그대로 옮겨온 것이 화근이었다.
...
**리빌드도 에디터도 필요 없다.** 콘솔에서 5분이면 원인이 갈린다.
